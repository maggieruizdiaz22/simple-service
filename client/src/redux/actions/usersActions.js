...
export const getUsers = () => async (dispatch) => {
  try {
    const response = await axios.get(
      "https://simple-service-production.up.railway.app/users"
    );
...
export const getAllUsers = () => async (dispatch) => {
  try {
    const response = await axios.get(
      `https://simple-service-production.up.railway.app/admin/users`,
      {
        headers: { Authorization: "Bearer " + token },
      }
    );
...
export const userLogin = async (token) => {
  const userResponseLogin = await axios.get(
    "https://simple-service-production.up.railway.app/login",
    {
      headers: { Authorization: "Bearer " + token },
    }
  );
  if (userResponseLogin.data.message !== "User succesfully logged in") {
    await axios.post("https://simple-service-production.up.railway.app/alta", {
